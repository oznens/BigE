...
# Statik Pages'te playback için küçük frontend düzeltmesi. Ana index.html'i
# çatallamak yerine build aşamasında eklenir; canlı scanner JS'i değişmez.
_PLAYBACK_PATCH = r"""
<script>
(function(){
  const _pbListe = pbRenderListe;
  pbRenderListe = function(tms){
    // Savunma katmanı: backend zaten filtreliyor; eski durum.json cache'i gelse
    // bile Expired/No-Entry vb. playback listesine asla girmesin.
  ...
  pbSec = async function(idx){
    ...
    try{
      ...
      // Seçilen tarihsel trade'in kendi seviyelerini grafiğe zorla. Böylece aynı
      // sembol/TF'de güncel setup varsa playback onu değil seçilen trade'i çizer.
      pbGrafVeri.seviye={
        ...(pbGrafVeri.seviye||{}),
      ...
      // Replay setup'ın hemen öncesinden başlasın; sonuç barına doğru ilerlesin.
...
def uret(cikti: Path, semboller: list[str], intervallar: list[str],
    ...
    # Playback, generic trade_memory'den bağımsız ve yalnız gerçek sonuçlar.
    ...
    for sym, ivl in hedefler:
        try:
            # Playback için daha geniş tarihsel pencere tut; frontend seçilen
            # trade'in açılış/kapanış zamanına göre replay aralığını belirler.
    ...
    # Patch'i body kapanmadan sonra yükle: orijinal playback fonksiyonlarını
    # kontrollü biçimde override eder.
    if "</body>" not in html:
